...
export function sign_out() {
  ...
  if (stacks_session) stacks_session.store.deleteSessionData();
  else localStorage.removeItem("blockstack-session");
}

export const stacks_connect_options = {
  appDetails: {
    name: "btc.nostrnames.org",
  ...
  onFinish: () => {
    ...
    if (!stacks_session.user_data) stacks_session.user_data = {};
    stacks_session.gaia = gaia_storage;
    session.set(stacks_session);
  },
};
