...
export async function get_base_chain_block_hash(transaction_id) {
  if (!tx_block_hash_cache[transaction_id]) {
    tx_block_hash_cache[transaction_id] = await json_safe(
      `base_block_hash_${transaction_id}`
    );
    if (!tx_block_hash_cache[transaction_id]) {
      tx_block_hash_cache[transaction_id] =
        await get_base_chain_block_hash_util(transaction_id);
      if (tx_block_hash_cache[transaction_id])
        json_safe(
          `base_block_hash_${transaction_id}`,
          tx_block_hash_cache[transaction_id]
...
export async function domain_status(domain) {
  ...
  if (!status) {
    ...
    if (api_status && api_status.domain) {
      ...
      if (status.transfer.txid) json_safe(file, status);
    } else return null;
  }
  return status;
}

export async function domain_status_from_tx_history_chunk(
  domain,
  address,
  limit,
  offset,
  ...
  for (let i = 0; i < transactions.length; ++i) {
    ...
    if (
      ...
      tx.contract_call.contract_id !== bns_contract
    )
      continue;
    if (
      !status.register &&
      tx.contract_call.function_name === "name-register"
    ) {
      const [namespace_arg, name_arg, salt_arg] =
        tx.contract_call.function_args;
      const namespace = Buffer.from(
        namespace_arg.repr.substr(2),
        "hex"
      ).toString("ascii");
      const name = Buffer.from(name_arg.repr.substr(2), "hex").toString(
        "ascii"
      );
      if (`${name}.${namespace}` !== `${domain}.${DOMAIN_NAMESPACE}`) continue;
      status.register = { txid: tx.tx_id };
      status.salt = Buffer.from(salt_arg.repr.substr(2), "hex").toString(
        "binary"
      );
    }
    if (
      status.register &&
      !status.preorder &&
      tx.contract_call.function_name === "name-preorder"
    ) {
      // find the first successful name-preorder after name-register.
      const hash = Buffer.from(
        tx.contract_call.function_args[0].repr.substr(2),
        "hex"
      );
      const expected_hash = await preorder_name_hash(
        `${domain}.${DOMAIN_NAMESPACE}`,
        Buffer.from(status.salt, "binary")
      );
      if (!hash.equals(expected_hash)) continue;
      status.preorder = { txid: tx.tx_id };
    }
    const ts = tx.burn_block_time * 1000;
    if (!status.ts || status.ts > ts) status.ts = ts;
    if (status.preorder && status.register) break;
  }
  return status;
}

export async function domain_transfer_status_from_nft_events(
  domain,
  address,
  limit,
  offset,
  ...
  for (let i = 0; i < events.length; ++i) {
    ...
    if (
      ...
      if (
        tuple &&
        tuple.data.name.buffer.toString("ascii") === domain &&
        tuple.data.namespace.buffer.toString("ascii") === DOMAIN_NAMESPACE
      ) {
...
export async function domain_status_from_tx_history(domain, address) {
  ...
  for (let offset = 0; !status.preorder || !status.register; ++offset) {
    let chunk = await domain_status_from_tx_history_chunk(
      domain,
      address,
      limit,
      offset * limit,
  ...
  if (!Object.keys(status).length) {
    for (let offset = 0; !status.transfer; ++offset) {
      let chunk = await domain_transfer_status_from_nft_events(
        domain,
        address,
        limit,
        offset * limit,
  ...
  if (Object.keys(status).length) json_safe(status_file_name(domain), status);
  return status;
}

export async function domain_purchase_prepare(domain) {
  domain = clean_check_domain(domain);
  if (!(await available(domain))) return false;
  const status_file = status_file_name(domain);
  let status = await json_safe(status_file);
  if (
    !status ||
    (status.step === STATUS_NEW && status.ts <= +new Date() - PREORDER_EXPIRY)
  ) {
    status = {
      step: STATUS_NEW,
      salt: Array.from(random_bytes(20)),
      ts: +new Date(),
    };
...
export async function domain_purchase_register(domain, nostr_pubkey) {
  domain = clean_check_domain(domain);
  const status_file = status_file_name(domain);
  let status = await json_safe(status_file);
  if (status.step === STATUS_PREORDER) {
    const zonefile = generate_zonefile_stub(
      domain,
      stacks_session &&
        stacks_session.user_data &&
        stacks_session.user_data.profile_url,
      nostr_pubkey
    );
    let tx_register = await register(domain, status.salt, zonefile);
    if (!tx_register) return status;
