...
const NODE_API =
  process.env.NETWORK === "testnet" ? TESTNET_NODE_API : MAINNET_NODE_API;
...
export function get_cookie(name, cookies) {
  ...
  if (kv) {
    ...
    if (values) return decodeURIComponent(values.trim());
  }
  return undefined;
}
...
export function set_cookie(name, value, options) {
  ...
  document.cookie = Object.entries(new_cookie)
    .map((kv) => kv.join("="))
    .join(";");
}
...
export const domain_fiat_price = readable("...", (set) => {
  ...
  let interval = setInterval(() => {
    if (
      !fetching &&
      (!latest.date || latest.date > Date.now() - refresh_rate)
    ) {
      fetching = true;
      fetch(STX_FIAT_PRICE_API)
        ...
        .then((json) => {
          if (json && json.blockstack && json.blockstack.usd) {
            ...
            set(
              "US$" + ((latest.price / 1000000) * DOMAIN_COST_STX).toFixed(2)
            );
        ...
        .catch(
          (error) =>
            process.env.DEV &&
            console.warn(
              "Failed to update STX price, will try again later.",
              error
            )
        )
...
export async function block_time() {
  try {
    let result = await fetch(
      "https://stacks-node-api.mainnet.stacks.co/extended/v1/info/network_block_times"
    );
    let json = await result.json();
    return process.env.NETWORK === "mainnet"
      ? json.mainnet.target_block_time
      : json.testnet.target_block_time;
  } catch (error) {
...
export async function start_stripe_session(domain_name, stx_address) {
  let result = await payment_api_call("new-stripe-session", {
    domain_name,
    stx_address,
  ...
  let stripe_redirect = await stripe.redirectToCheckout({
    sessionId: result.session_id,
  });
  if (stripe_redirect.error) console.error(stripe_redirect.error);
  return !!stripe_redirect.error;
}

export async function start_coinbase_session(domain_name, stx_address) {
  let result = await payment_api_call("new-coinbase-session", {
    domain_name,
    stx_address,
    return_to: document.location.href,
  });
  ...
  document.location.href =
    result.session_url ||
    `https://commerce.coinbase.com/charges/${result.session_id}`;
}
...
export async function get_base_chain_block_hash(transaction_id) {
  ...
  if (!json1 || !json1.block_hash) return false;
  const call2 = await fetch(
    `${NODE_API}/extended/v1/block/${json1.block_hash}`
  );
...
export async function get_address_transactions(address, limit, offset) {
  const body = await fetch(
    `${NODE_API}/extended/v1/address/${address}/transactions?limit=${
      limit || 30
    }&offset=${offset || 0}`
  );
  const json = await body.json();
...
export async function get_address_nft_events(address, limit, offset) {
  const body = await fetch(
    `${NODE_API}/extended/v1/address/${address}/nft_events?limit=${
      limit || 30
    }&offset=${offset || 0}`
  );
  const json = await body.json();
...
export async function get_address_stx_balance(address) {
  const body = await fetch(
    `${NODE_API}/extended/v1/address/${address}/balances`
  );
  const json = await body.json();
...
export function deserializeCVHex(hex) {
  ...
  return deserializeCV(
    Buffer.from(hex[1] === "x" ? hex.substr(2) : hex, "hex")
  );
}
