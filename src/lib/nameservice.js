...
export let BNS_CONTRACT_ADDRESS =
...
export async function preorder_name_hash(name, salt) {
  return hash160(
    Buffer.concat([
      Buffer.from(name),
      typeof salt !== "string" ? salt : Buffer.from(salt),
    ])
  );
}
...
async function contract_write(func, args, post_conditions, attachment) {
  ...
  if (!Array.isArray(post_conditions))
    post_conditions = typeof post_conditions === "number" &&
      post_conditions > 0 && [
        makeStandardSTXPostCondition(
          address,
          FungibleConditionCode.Equal,
          new BN(post_conditions)
        ),
      ];
...
export async function zonefile(domain) {
  ...
  if (!zone_object["$origin"]) zone_object["$origin"] = `${domain}.`;
  if (!zone_object["$ttl"]) zone_object["$ttl"] = 3600;
  return zone_object;
}
...
export async function preorder(domain, salt) {
  ...
  if (length !== 20) throw new Error("Salt should be 20 bytes.");
  salt = Buffer.from(salt, "binary");
  domain = clean_check_domain(domain);
  let domain_hash = await preorder_name_hash(
    `${domain}.${DOMAIN_NAMESPACE}`,
    salt
  );
  return await contract_write(
    "name-preorder",
    [bufferCVFromString(domain_hash), uintCV(DOMAIN_COST_STX)],
    DOMAIN_COST_STX
  ); // + NAME_PREORDER_TX_FEE
}
...
export async function register(domain, salt, zonefile) {
  ...
  return await contract_write(
    ...
    [
      bufferCVFromString(DOMAIN_NAMESPACE),
      bufferCVFromString(clean_check_domain(domain)),
      bufferCV(Buffer.from(salt, "binary")),
...
export async function revoke(domain) {
  return await contract_write("name-revoke", [
    bufferCVFromString(DOMAIN_NAMESPACE),
    bufferCVFromString(clean_check_domain(domain)),
  ]);
}
...
export async function transfer(domain, new_owner, zonefile) {
  ...
  let hash = await zonefile_hash(zonefile);
  let asset_name = tupleCV({
    name: bufferCVFromString(domain),
    namespace: bufferCVFromString(DOMAIN_NAMESPACE),
  });
  ...
  return await contract_write(
    ...
    [
      bufferCVFromString(DOMAIN_NAMESPACE),
      bufferCVFromString(domain),
      standardPrincipalCV(new_owner),
      someCV(bufferCVFromString(hash)),
    ],
    [
      makeStandardNonFungiblePostCondition(
        address,
